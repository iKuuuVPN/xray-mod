...
struct ss_mb_ctx {
	...
	uint8_t hkdf_out[SS_MB_MAX_BURST][IMB_SHA1_DIGEST_SIZE_IN_BYTES];
...
static int ss_mb_match_aes128gcm(struct ss_mb_ctx *ctx,
	...
	static const uint8_t info_msg[10] = {
	...
	for (size_t base = 0; base < n_keys; base += SS_MB_MAX_BURST) {
		...
		for (size_t i = 0; i < n; i++) {
			...
			job->src = info_msg;
			job->hash_start_src_offset_in_bytes = 0;
			job->msg_len_to_hash_in_bytes = sizeof(info_msg);
			job->auth_tag_output = ctx->hkdf_out[i];
			job->auth_tag_output_len_in_bytes = IMB_SHA1_DIGEST_SIZE_IN_BYTES;
		}
		...
		for (size_t i = 0; i < n; i++) {
			IMB_AES128_GCM_PRE(ctx->mgr, ctx->hkdf_out[i], &ctx->gcm_keys[i]);
...
func (m *AES128GCMUserMatcher) MatchTCPIPsecMB(bs []byte) (u *protocol.MemoryUser, aead cipher.AEAD, ivLen int32, err error) {
	const (
		aes128gcmIVLen = 16
	)
	...
	if len(bs) < aes128gcmIVLen+2+16 {
		return nil, nil, 0, ErrNotFound
	}

	salt := bs[:aes128gcmIVLen]
	ct2 := bs[aes128gcmIVLen : aes128gcmIVLen+2]
	tag := bs[aes128gcmIVLen+2 : aes128gcmIVLen+2+16]
	...
	case 0:
		...
		key := m.keysFlat[idx*16 : idx*16+16]
		var subkey [16]byte
		hkdfSHA1(key, salt, subkey[:])
		aead = createAesGcm(subkey[:])
		return m.users[idx], aead, aes128gcmIVLen, nil
	case 1:
		return nil, nil, 0, ErrNotFound
	default:
